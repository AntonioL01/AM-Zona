...
function UploadImages({ triggleUploadImages, setLoader, carInfo, mode }) {
  ...
  const onImageRemoveFromDB = async (image) => {
    try {
      const response = await fetch('/.netlify/functions/manage-images', {
        method: 'POST',
        body: JSON.stringify({ action: 'remove', imageId: image.id }),
      });
      if (response.ok) {
  ...
  const UploadImageToServer = async () => {
    ...
    for (const file of selectedFileList) {
      ...
      if (downloadUrl && triggleUploadImages) {
        ...
        await fetch('/.netlify/functions/manage-images', {
          method: 'POST',
          body: JSON.stringify({ action: 'add', imageUrl: downloadUrl, carListingId: triggleUploadImages }),
        });
